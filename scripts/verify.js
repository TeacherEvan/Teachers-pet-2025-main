...
const forbiddenPatterns = [
  {
    pattern: /return `.*(K1|K2|August|November).*`/g,
    message: "Potential hardcoded grade/month in template return string",
  },
  {
    pattern: /const opening = \".*(K1|K2|August|November).*\"/g,
    message: "Potential hardcoded grade/month in opening variable",
  },
];
...
if (fs.existsSync(templatesPath)) {
  ...
  forbiddenPatterns.forEach(({ pattern, message }) => {
    if (pattern.test(content)) {
      log(
        "warn",
        `${message} detected in templates.js. Verify this is intentional.`,
      );
...
jsFilesToCheck.forEach((file) => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    if (content.includes("console.log")) {
      log(
        "warn",
        `Console.log detected in ${file}. Ensure this is for debugging only.`,
      );
    }
  }
});
...
if (hasError) {
  console.log(
    "\n" +
      COLORS.red +
      "Verification FAILED. Please fix the errors above before committing." +
      COLORS.reset,
  );
  process.exit(1);
} else {
  console.log(
    "\n" +
      COLORS.green +
      "Verification PASSED. You are good to commit!" +
      COLORS.reset,
  );
  process.exit(0);
}
