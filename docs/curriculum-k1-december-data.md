# K1 December 2025 Curriculum - Marking Points

**Grade:** K1/1  
**Month:** December 2025  
**Semester:** 2nd Semester

---

## English

**Activities:**

- ☐ Trace the letters following the dotted line and color the letters
- ☐ Draw lines from the letter to the picture starting from the dot
- ☐ Draw lines to match the same letter
- ☐ Draw lines to match the letter with the correct picture

**Vocabulary:** A-Ant, B-Bee, C-Cat, D-Dog, E-Egg, F-Fan, H-Hen, I-Ice Cream, O-Ox, T-Tea

---

## Mathematics

**Activities:**

- ☐ Write numbers 1-6 following the lines and write freehand in the box
- ☐ Draw the correct amount of circles in the box next to the numbers
- ☐ Trace number 6 following the dotted line and colour
- ☐ Write numbers 1-7 following the lines and write freehand in the box
- ☐ Draw the correct amount of circles in the box next to the number
- ☐ Color the correct amount of pictures and cut the pictures and paste them in the boxes

**Vocabulary:** One – ten

---

## I.Q (Intelligence/Logic)

**Activities:**

- ☐ Object and Animal - Circle the animals
- ☐ Direction (Above) - Cross the objects that are above
- ☐ Objects in the same group - Draw lines to match objects that are further
- ☐ Mark X to objects that are further
- ☐ Draw lines to match suitable size objects
- ☐ Direction: Front and back - Circle the pictures that face the front - Cross the pictures that face the back
- ☐ Animal (Wings) - Circle the animals that have wings
- ☐ Incomplete – Mark the incomplete objects with X

**Vocabulary:** Car, giraffe, girl, cat, telephone, alarm clock, vase/flower, toothbrush/toothpaste, pot/stove, bird, duck, hen, owl, lion, turtle, dog, book, boat, small girl, small dress, big girl, big dress, pencil, van, frog, rice cooker, swing, briefcase

---

## Social Studies

**Activities:**

- ☐ Kindness to the Environment- Discuss what is the environment and taking care of it
- ☐ Benefits of Trees – Explaining the effects of cutting down trees and destroying the forest
- ☐ Insects -Mark the insects with X
- ☐ Sea Animals –Circle the animals that live in the ocean

**Vocabulary:** environment, water, plants, feed, animals, throw, rubbish, tree, forest, deforestation, leaves, trunk, fruits, insects, ant, butterfly, mosquito, ladybug, dragonfly, grasshopper, fish, sea, shark, starfish, dolphin, whale, mussel, ocean

---

## Phonics

**Activities:**

- ☐ Letter R (Ricky rabbit, read)
- ☐ Letter S (Susie seal, sun)
- ☐ Rev Q/R/S (Review)
- ☐ Letter T (Teddy tiger, teeth)
- ☐ Letter U (Uncle utter, under)

**Vocabulary:** Ricky rabbit, read, Susie seal, sun, quiet, on, tiger, uncle, Teddy tiger, teeth, Uncle utter, under

---

## Science

**Activities:**

- ☐ Mixing colours - Students learn about primary and secondary colours by mixing coloured water
- ☐ Bottle diver experiment - Introduce K1 students to the concepts of buoyancy and density through a hands-on activity using a simple bottle and paper-clip
- ☐ Air pressure - Introduce K1 students to the concept of air pressure through a simple experiment using a candle and a bottle
- ☐ Air rocket - Introduce K1 students to the concept of air pressure of moving objects by pressure of a rocket toy

**Vocabulary:** Water, colour, mix, bowl, bottle, water, paper, paper-clip, food colouring, candle, air, sound, table, pencil, air rocket, stepping hard, press/push, fly up

---

## Conversation 1

**Topics:**

- ☐ Colours Question- What colour is the banana/tree? Answer- The ... is yellow/green
- ☐ Family Members Question- Who do you love? Answer- I love my father and mother
- ☐ Abilities Question- What can you do? Answer- I can sing/dance
- ☐ Playground Equipment Question- Where do you like playing? Answer- I like playing on the seesaw/slide

**Vocabulary:** Green, yellow, red, blue, mother, father, sing, dance, seesaw, slide

---

## Conversation 3

**Topics:**

- ☐ Food: Actions -What are you doing? I am walking
- ☐ Weather- How's the weather? It's sunny

**Vocabulary:** walking, running, jumping, sitting, sunny, rainy, cloudy

---

## Arts

**Activities:**

- ☐ Butterfly squish painting
- ☐ Sponge painting tree design
- ☐ New year card making
- ☐ Origami - house

**Vocabulary:** fold, paper, squish, paint, butterfly, flower, sponge, stem, new year, January, card, house, window, roof

---

## Physical Education

**Activities:**

- ☐ Hopscotch
- ☐ Picking Up
- ☐ Matching & Dropping Ball Race Jumping
- ☐ Dart Ball Game
- ☐ Cart Racing

**Vocabulary:** Hop, number, match, ball, chair, colour, dart board, throw, cart, push, race

---

## Puppet Show

**Stories:**

- ☐ The Ants and the Grasshopper
- ☐ Lion and the Mouse

**Vocabulary:** ant, grasshopper, hungry, winter, food, lion, mouse, forest, net

---

## Summary

**Total Subjects:** 11

- English
- Mathematics
- I.Q
- Social Studies
- Phonics (NEW for December)
- Science
- Conversation 1
- Conversation 3
- Arts
- Physical Education
- Puppet Show

**Key Themes:** Letter recognition (A-T), Numbers 1-10, Environmental awareness, Phonics introduction (R, S, T, U), Social interaction, Creative arts, Physical development

**Notes:**

- December introduces **Phonics** as a new subject separate from English
- Focus on environmental themes (trees, ocean animals, caring for nature)
- Conversation 3 emphasizes daily activities and weather
- Arts activities focus on seasonal themes (New Year)
