...
export const TeachersPetTemplates = {
  ...
  generateStyleComment: async function (style, data) {
    ...
    // 3. Subject-specific progress with topics (or orphaned topics)
    const hasSubjectInfo =
      (data.subjects && data.subjects.length > 0) ||
      (data.orphanedTopics && data.orphanedTopics.length > 0);
    if (hasSubjectInfo) {
  ...
  generateSubjectSection: function (data, isMale) {
    ...
    const subjectsWithTopics = Object.entries(topicsBySubject).filter(
      ([subject, topics]) => topics.length > 0,
    );
    ...
    if (remainingSubjects.length > 0) {
      const subjectsList = TeachersPetUtils.naturalJoin(
        remainingSubjects.map((s) => `'${s}'`),
      );
      if (isMale) {
        parts.push(
          `${data.pronoun_subject} also made consistent and ${data.descriptor} progress in ${subjectsList}, demonstrating versatile aptitude.`,
        );
      } else {
        parts.push(
          `${data.pronoun_subject} also progressed well in ${subjectsList}, engaging positively with each activity.`,
        );
      }
    }
    ...
    if (parts.length === 0 && allSubjects.length > 0) {
      const allSubjectsList = TeachersPetUtils.naturalJoin(
        allSubjects.map((s) => `'${s}'`),
      );
      if (isMale) {
        parts.push(
    ...
    // Orphaned topics (mention without inventing subjects)
    if (data.orphanedTopics && data.orphanedTopics.length > 0) {
      const orphanList = TeachersPetUtils.naturalJoin(data.orphanedTopics);
      if (isMale) {
        parts.push(
          `${data.name} also covered additional topics such as ${orphanList}, demonstrating continued engagement.`,
        );
      } else {
        parts.push(
          `${data.name} also explored additional topics such as ${orphanList} with positive interest.`,
        );
      }
    }
